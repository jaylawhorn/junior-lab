...
void threePulseTest(TString conf="threePulse") {
  ...
  echo_height->GetXaxis()->SetTitle("#tau [s]");
  echo_height->GetYaxis()->SetTitle("Spin Echo Amplitude [mV]");
  echo_height->GetYaxis()->SetRangeUser(0, 20);

  TF1 *t1fit2 = new TF1("t1fit2", "[0]-expo(1)", 15, 70);
  TF1 *t1fit = new TF1("t1fit", "[0]-expo(1)", 10, 70);
  t1fit->SetLineColor(kBlue);
  t1fit2->SetLineColor(kBlue);
  ...
  cout << "T1 = " << -1.0/t1fit->GetParameter(2) << " +- " << unc << " +- " << fabs(t12-t1) << endl;
  cout << "     " << TMath::Sqrt(unc*unc+fabs(t12-t1)*fabs(t12-t1)) << endl;
  cout << "T1 = " << -1.0/t1fit2->GetParameter(2) << " +- " << unc2 << " +- " << fabs(t12-t1) << endl;
  cout << "     " << TMath::Sqrt(unc2*unc2+fabs(t12-t1)*fabs(t12-t1)) << endl;

  echo_height->SetTitle("");
  echo_height->GetXaxis()->SetTitle("Tau [ms]");
  echo_height->GetYaxis()->SetTitle("Spin Echo Amplitude [mV]");
  echo_height->Draw("ap");

  //c1->SaveAs("images/threePulse.png");
